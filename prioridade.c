...
void prioridade(struct BCP process[], int n_process) {
printf("\n\t\tAlgoritmo escolhido: Prioridade não-preemptivo\n");
...
//Verificando e ordenando por ordem de prioridade (menor número, maior prioridade)
for(i=0; i<n_process; i++){
  for(j=i+1; j<n_process; j++){
    if(process[i].temp_cheg == process[j].temp_cheg){
      if(process[i].prioridade > process[j].prioridade){
        aux[i]= process[i];
        process[i]= process[j];
        process[j] = aux[i];
